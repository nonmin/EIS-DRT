...
R_infy_simu = 5; %ohm, high-frequency cut-off resistance
R_p_simu = 40; %ohm, overall polarization resisitance
t_0 = 0.01; % s, characteristic time constant
phi = 0.55; % in [0,1]; controlling the width of analytical drt

% analytical impedace data
Z_simu = R_infy_simu + R_p_simu./(1+power(2i*pi*t_0.*f,phi));
...
% analytical drt
DRT_simu = R_p_simu/2/pi*sin((1-phi)*pi)./(cosh(phi.*log(tc./t_0))-cos((1-phi)*pi));
