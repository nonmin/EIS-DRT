...
R2_simu = 50; %ohm
%R2_simu = 30; %ohm
t1 = 0.001; % s, characteristic time constant
t2 = 0.1;
phi = 0.7; % in [0,1]; controlling the width of analytical drt

% analytical impedace data
Z_simu = R_infy_simu + R1_simu./(1+power(2i*pi*t1.*f,phi)) + R2_simu./(1+power(2i*pi*t2.*f,phi));
...
% analytical drt
DRT_simu = R1_simu/2/pi*sin((1-phi)*pi)./(cosh(phi.*log(tc./t1))-cos((1-phi)*pi))+...
           R2_simu/2/pi*sin((1-phi)*pi)./(cosh(phi.*log(tc./t2))-cos((1-phi)*pi));
